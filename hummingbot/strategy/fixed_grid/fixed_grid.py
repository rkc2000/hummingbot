...
from hummingbot.core.event.events import BuyOrderCompletedEvent, SellOrderCompletedEvent, OrderFilledEvent
from hummingbot.strategy.utils import order_age
from hummingbot.strategy.order_tracker import OrderTracker
from hummingbot.strategy.fixed_grid.data_types import (
    PriceSize,
    Proposal,
)
...
class FixedGridStrategy(StrategyPyBase):
    ...
    def init_params(self,
                    ...
                    spread_scale_factor: Decimal,
                    amount_scale_factor: Decimal,
        ...
        if n_levels % 2 != 0:
            n_levels = n_levels - 1
            self.logger().info("n_levels must be an even number. Reducing input value by 1")
        ...
        self._spread_scale_factor = spread_scale_factor
        self._amount_scale_factor = amount_scale_factor
    ...
    @property
    def spread_scale_factor(self) -> Decimal:
        return self._spread_scale_factor

    @spread_scale_factor.setter
    def spread_scale_factor(self, value: Decimal):
        self._spread_scale_factor = value

    @property
    def amount_scale_factor(self) -> Decimal:
        return self._amount_scale_factor

    @amount_scale_factor.setter
    def amount_scale_factor(self, value: Decimal):
        self._amount_scale_factor = value
    ...
    @property
    def order_amount_levels(self) -> List:
        return self._order_amount_levels
    ...
    @property
    def quote_inv_levels_current_price(self) -> List:
    ...
    def start(self, clock: Clock, timestamp: float):
        minimum_spread = (self._grid_price_ceiling - self._grid_price_floor)/(1 + 2*sum([pow(self._spread_scale_factor, n) for n in range(1,int(self._n_levels/2))]))

        self._price_levels.append(self._grid_price_floor)
        for i in range(2, int(self._n_levels/2)+1):
            price = self._grid_price_floor + minimum_spread*sum([pow(self._spread_scale_factor, int(self._n_levels/2) - n) for n in range(1,i)])
            self._price_levels.append(price)
        for i in range(1, int(self._n_levels/2)+1):
            self._order_amount_levels.append(self._order_amount*pow(self._amount_scale_factor,int(self._n_levels/2)-i))

        for i in range(int(self._n_levels/2)+1, self._n_levels+1):
            price = self._price_levels[int(self._n_levels/2)-1] + minimum_spread*sum([pow(self._spread_scale_factor, n) for n in range(0,i-int(self._n_levels/2))])
            self._price_levels.append(price)
            self._order_amount_levels.append(self._order_amount*pow(self._amount_scale_factor, i-int(self._n_levels/2)-1))

        for i in range(1, self._n_levels+1):
            self._base_inv_levels.append(sum(self._order_amount_levels[i:self._n_levels]))
            self._quote_inv_levels.append(sum([self._price_levels[n]*self._order_amount_levels[n] for n in range(0,i-1)]))
        for i in range(self._n_levels):
            self._quote_inv_levels_current_price.append(self._quote_inv_levels[i]/self._price_levels[i])

        print(self._price_levels)
        print(self._order_amount_levels)
        print(self._base_inv_levels)
        print(self._quote_inv_levels)

        self._last_timestamp = timestamp

    # def stop(self, clock: Clock):
    #     StrategyPyBase.stop(self, clock)
    ...
    def create_grid_proposal(self):
        ...
        for i in range(self._current_level):
            ...
            size = self._order_amount_levels[i]
    ...
    def apply_order_optimization(self, proposal: Proposal):
        ...
        if len(proposal.buys) > 0:
            ...
            for i, proposed in enumerate(proposal.buys):
                proposal.buys[i].price = market.quantize_order_price(self.trading_pair, lower_buy_price) * (1 - self.order_level_spread * i)
        ...
        if len(proposal.sells) > 0:
            ...
            for i, proposed in enumerate(proposal.sells):
                proposal.sells[i].price = market.quantize_order_price(self.trading_pair, higher_sell_price) * (1 + self.order_level_spread * i)
    ...
    def did_complete_buy_order(self, event: BuyOrderCompletedEvent):
        ...
        if self._inv_correct is False:
            ...
            self._create_timestamp = self.current_timestamp + self._order_refresh_time
        ...
        if self._inv_correct is True:
            ...
            price = market.quantize_order_price(self.trading_pair, price)
            size = self._order_amount_levels[self._current_level+1]
            size = market.quantize_order_amount(self.trading_pair, size)
            self.execute_orders_proposal(Proposal([], [PriceSize(price, size)]))
    ...
    def cancel_active_orders_on_max_age_limit(self):
        ...
        if active_orders and any(order_age(o, self.current_timestamp) > self._max_order_age for o in active_orders):
